...
def _already_analyzed(out_path, latest_round):
    """同じ最新回号で分析済みなら True。読めない・壊れている場合は False。"""
    if not out_path.exists():
        return False
    try:
        with open(out_path, "r", encoding="utf-8") as f:
            return json.load(f).get("latest_round") == latest_round
    except (OSError, ValueError):
        return False


def run(game: str, force: bool = False):
    game_cfg = GAMES[game]
    D = game_cfg["digits"]
    ...
    # 新しい抽選が無ければ再計算しない。LSTMが実行ごとに微妙にぶれるため、
    # 再計算すると同じ回号なのに予想が変わってしまう（見て買った番号と記録がズレる）。
    # 最初に出した予想をそのまま残すのが目的なので、出力に一切触れずに戻る。
    if not force and _already_analyzed(game_cfg["output_path"], latest_round):
        print(f"第{latest_round}回まで分析済み。新しい抽選が無いのでスキップします"
              f"（分析ロジックを変えた時は --force）")
        return
